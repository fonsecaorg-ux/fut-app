...
def sugerir_mercados_hedge_inteligente(bilhete_principal: List[Dict], 
                                        jogo: str, 
                                        stats: Dict,
                                        calc: Dict) -> List[Dict]:
    """
    HEDGE SUPER INTELIGENTE - Analisa base de dados e sugere hedges baseados em estatísticas reais
    
    Regras:
    1. NUNCA sugerir gols
    2. PRIORIDADE: Escanteios > Cartões > Dupla Chance (último recurso)
    3. Analisa dados históricos dos times específicos
    4. Só sugere se probabilidade >= 65% e faz sentido estatisticamente
    """
    ...
    # Extrair mercados do bilhete principal
    mercados_principais = [aposta['mercado'] for aposta in bilhete_principal if aposta['jogo'] == jogo]
    ...
    # Escanteios Casa - Verificar se time da casa tem bom histórico
    corners_casa = stats_casa.get('corners', 0)
    if corners_casa > 0:
        # Analisar linhas de cantos CASA
        for linha in [2.5, 3.5, 4.5, 5.5]:
            mercado = f"Over {linha} Cantos Casa"
            
            # Não sugerir se já está no bilhete
            if mercado in mercados_principais:
                continue
            
            # Calcular probabilidade baseada em dados históricos
            prob = calcular_poisson(calc['corners_home'], linha)
            
            # Critério: Média alta + probabilidade >= 65%
            if corners_casa >= linha * 0.85 and prob >= 65:
                confiabilidade = "ALTA" if corners_casa >= linha else "MÉDIA"
                sugestoes.append({
                    'mercado': mercado,
                    'prob': prob,
                    'media_historica': corners_casa,
                    'analise': f"{time_casa} faz {corners_casa:.1f} cantos/jogo em casa",
                    'confiabilidade': confiabilidade,
                    'tipo': 'ESCANTEIO_CASA'
                })
    
    # Escanteios Fora - Verificar time visitante
    corners_fora = stats_fora.get('corners', 0)
    if corners_fora > 0:
        for linha in [2.5, 3.5, 4.5]:
            mercado = f"Over {linha} Cantos Fora"
            
            if mercado in mercados_principais:
                continue
            
            prob = calcular_poisson(calc['corners_away'], linha)
            
            if corners_fora >= linha * 0.85 and prob >= 65:
                confiabilidade = "ALTA" if corners_fora >= linha else "MÉDIA"
                sugestoes.append({
                    'mercado': mercado,
                    'prob': prob,
                    'media_historica': corners_fora,
                    'analise': f"{time_fora} faz {corners_fora:.1f} cantos/jogo fora",
                    'confiabilidade': confiabilidade,
                    'tipo': 'ESCANTEIO_FORA'
                })
    
    # Escanteios Totais - Se nenhum dos times individualmente for bom, verificar total
    corners_total_historico = corners_casa + corners_fora
    for linha in [7.5, 8.5, 9.5]:
        mercado = f"Over {linha} Cantos Total"
        
        if mercado in mercados_principais:
            continue
        
        prob = calcular_poisson(calc['corners_total'], linha)
        
        if corners_total_historico >= linha * 0.90 and prob >= 65:
            confiabilidade = "ALTA" if corners_total_historico >= linha * 1.1 else "MÉDIA"
            sugestoes.append({
                'mercado': mercado,
                'prob': prob,
                'media_historica': corners_total_historico,
                'analise': f"Média combinada: {corners_total_historico:.1f} cantos",
                'confiabilidade': confiabilidade,
                'tipo': 'ESCANTEIO_TOTAL'
            })
    ...
    # Cartões Casa
    cards_casa = stats_casa.get('cards', 0)
    if cards_casa > 0:
        for linha in [1.5, 2.5]:
            mercado = f"Over {linha} Cartões Casa"
            
            if mercado in mercados_principais:
                continue
            
            prob = calcular_poisson(calc['cards_home'], linha)
            
            if cards_casa >= linha * 0.90 and prob >= 65:
                confiabilidade = "ALTA" if cards_casa >= linha else "MÉDIA"
                sugestoes.append({
                    'mercado': mercado,
                    'prob': prob,
                    'media_historica': cards_casa,
                    'analise': f"{time_casa} leva {cards_casa:.1f} cartões/jogo em casa",
                    'confiabilidade': confiabilidade,
                    'tipo': 'CARTAO_CASA'
                })
    
    # Cartões Fora
    cards_fora = stats_fora.get('cards', 0)
    if cards_fora > 0:
        for linha in [1.5, 2.5]:
            mercado = f"Over {linha} Cartões Fora"
            
            if mercado in mercados_principais:
                continue
            
            prob = calcular_poisson(calc['cards_away'], linha)
            
            if cards_fora >= linha * 0.90 and prob >= 65:
                confiabilidade = "ALTA" if cards_fora >= linha else "MÉDIA"
                sugestoes.append({
                    'mercado': mercado,
                    'prob': prob,
                    'media_historica': cards_fora,
                    'analise': f"{time_fora} leva {cards_fora:.1f} cartões/jogo fora",
                    'confiabilidade': confiabilidade,
                    'tipo': 'CARTAO_FORA'
                })
    
    # Cartões Totais
    cards_total_historico = cards_casa + cards_fora
    for linha in [2.5, 3.5, 4.5]:
        mercado = f"Over {linha} Cartões Total"
        
        if mercado in mercados_principais:
            continue
        
        prob = calcular_poisson(calc['cards_total'], linha)
        
        if cards_total_historico >= linha * 0.85 and prob >= 65:
            confiabilidade = "ALTA" if cards_total_historico >= linha * 1.1 else "MÉDIA"
            sugestoes.append({
                'mercado': mercado,
                'prob': prob,
                'media_historica': cards_total_historico,
                'analise': f"Média combinada: {cards_total_historico:.1f} cartões",
                'confiabilidade': confiabilidade,
                'tipo': 'CARTAO_TOTAL'
            })
    
    # ==========================================
    # DUPLA CHANCE (ÚLTIMO RECURSO)
    # ==========================================
    
    # Só sugere Dupla Chance se não houver boas opções de cantos/cartões
    if len(sugestoes) < 2:
        # Análise de força dos times baseado em gols feitos/sofridos
        gols_casa_feitos = stats_casa.get('goals_f', 0)
        gols_casa_sofridos = stats_casa.get('goals_a', 0)
        gols_fora_feitos = stats_fora.get('goals_f', 0)
        gols_fora_sofridos = stats_fora.get('goals_a', 0)
        
        # Time da casa é forte?
        if gols_casa_feitos > gols_fora_feitos * 1.3:
            sugestoes.append({
                'mercado': '1X (Casa ou Empate)',
                'prob': 75,  # Estimativa conservadora
                'media_historica': 0,
                'analise': f"{time_casa} mais forte em casa (gols: {gols_casa_feitos:.1f} vs {gols_fora_feitos:.1f})",
                'confiabilidade': 'MÉDIA',
                'tipo': 'DUPLA_CHANCE'
            })
        # Time de fora é competitivo?
        elif gols_fora_feitos >= gols_casa_feitos * 0.8:
            sugestoes.append({
                'mercado': 'X2 (Empate ou Fora)',
                'prob': 70,
                'media_historica': 0,
                'analise': f"{time_fora} competitivo fora (gols: {gols_fora_feitos:.1f})",
                'confiabilidade': 'MÉDIA',
                'tipo': 'DUPLA_CHANCE'
            })
    ...
    # Priorizar: Confiabilidade ALTA > Probabilidade > Tipo (Escanteio > Cartão > Dupla Chance)
    def score_sugestao(sug):
        score = sug['prob']  # Base: probabilidade
        
        # Bonus por confiabilidade
        if sug['confiabilidade'] == 'ALTA':
            score += 10
        
        # Bonus por tipo (priorizar escanteios e cartões)
        if 'ESCANTEIO' in sug['tipo']:
            score += 5
        elif 'CARTAO' in sug['tipo']:
            score += 3
        # Dupla chance não ganha bonus
        
        return score
    
    sugestoes.sort(key=score_sugestao, reverse=True)
...
def calcular_probabilidade_mercado(mercado: str, calc: Dict) -> float:
    ...
    mercado_map = {
        ...
        "Over 4.5 Cantos Fora": ('corners_away', 4.5),
        "Over 5.5 Cantos Fora": ('corners_away', 5.5),
        ...
        # CARTÕES CASA
        "Over 1.5 Cartões Casa": ('cards_home', 1.5),
        "Over 2.5 Cartões Casa": ('cards_home', 2.5),
        
        # CARTÕES FORA
        "Over 1.5 Cartões Fora": ('cards_away', 1.5),
        "Over 2.5 Cartões Fora": ('cards_away', 2.5),
    }
...
def main():
                ...
                for _, jogo in jogos.iterrows():
                    ...
                    if h and a:
                        ...
                        with st.expander(f"⚽ {h} vs {a} | 🕒 {jogo.get('Hora', '-')}"):
                            ...
                            m1.metric("🚩 Cantos Total", f"{calc['corners_total']:.1f}")
                            m2.metric("⚽ Gols", f"{calc['total_goals']:.1f}")
                            m3.metric("📋 Cartões Total", f"{calc['cards_total']:.1f}")
                            
                            st.markdown("**🎯 Quick Add:**")
                            b1, b2, b3 = st.columns(3)
                            
                            if b1.button("➕ Over 8.5 Cantos", key=f"c85_{h}_{a}"):
                                prob = calcular_poisson(calc['corners_total'], 8.5)
                                st.session_state.current_ticket.append({
                                    'jogo': f"{h} vs {a}",
                                    'mercado': 'Over 8.5 Cantos Total',
                                    'odd': 1.85,
                                    'prob': prob,
                                    'tipo': 'Auto'
                                })
                            ...
                            if b3.button("➕ Over 2.5 Cartões", key=f"ca25_{h}_{a}"):
                                prob = calcular_poisson(calc['cards_total'], 2.5)
                                st.session_state.current_ticket.append({
                                    'jogo': f"{h} vs {a}",
                                    'mercado': 'Over 2.5 Cartões Total',
                                    'odd': 1.75,
                                    'prob': prob,
                                    'tipo': 'Auto'
                                })
    ...
    # TAB 2: HEDGE INTELIGENTE
    with tabs[1]:
        st.header("🛡️ Sistema de Hedge Inteligente")
        
        if not st.session_state.current_ticket:
            st.warning("⚠️ Crie um bilhete primeiro na aba Construtor!")
        else:
            # ===== BILHETE PRINCIPAL =====
            st.markdown("### 📋 Seu Bilhete Principal")
            ...
            with st.container():
                ...
                # Mostrar apostas do bilhete
                for idx, aposta in enumerate(st.session_state.current_ticket, 1):
                    col1, col2, col3 = st.columns([3, 2, 2])
                    col1.markdown(f"**{idx}. {aposta['jogo']}**")
                    col2.markdown(f"📊 {aposta['mercado']}")
                    col3.markdown(f"🎯 Odd {aposta['odd']:.2f} | {aposta['prob']:.0f}%")
                
                st.markdown("---")
                
                # Métricas do bilhete
                odd_total = np.prod([x['odd'] for x in st.session_state.current_ticket])
                prob_total = np.prod([x['prob']/100 for x in st.session_state.current_ticket]) * 100
                
                m1, m2, m3 = st.columns(3)
                m1.metric("📊 Odd Total", f"{odd_total:.2f}")
                m2.metric("🎯 Prob. Total", f"{prob_total:.1f}%")
                m3.metric("📈 EV%", f"{((odd_total * prob_total/100) - 1) * 100:.1f}%")
            
            st.markdown("---")
            
            # ===== SISTEMA DE HEDGE INTELIGENTE =====
            st.markdown("### 🧠 Hedge Inteligente (Baseado em Dados)")
            
            st.info("""
            **Como funciona:**
            1. 🔍 Analisa as estatísticas reais dos times no seu bilhete
            2. 🧮 Calcula probabilidades baseadas em dados históricos
            3. 🎯 Sugere hedges DIFERENTES do seu bilhete principal
            4. ✅ Prioriza: Escanteios > Cartões > Dupla Chance
            5. ❌ NUNCA sugere gols
            """)
            
            if st.button("🚀 GERAR HEDGES INTELIGENTES", use_container_width=True, type="primary"):
                with st.spinner("🧠 Analisando base de dados..."):
                    time.sleep(0.5)
                    
                    # Agrupar apostas por jogo
                    jogos_unicos = list(set([a['jogo'] for a in st.session_state.current_ticket]))
                    ...
                    for jogo in jogos_unicos:
                        try:
                            ...
                            sugestoes = sugerir_mercados_hedge_inteligente(
                                st.session_state.current_ticket,
                                jogo,
                                STATS,
                                calc
                            ...
                            if sugestoes:
                                todas_sugestoes[jogo] = sugestoes
                        
                        except Exception as e:
                            continue
                    
                    # Armazenar no session_state
                    st.session_state.hedge_sugestoes = todas_sugestoes
                    st.success("✅ Análise concluída!")
            
            # ===== MOSTRAR SUGESTÕES =====
            if hasattr(st.session_state, 'hedge_sugestoes') and st.session_state.hedge_sugestoes:
                st.markdown("---")
                st.markdown("### 💡 Sugestões de Hedge")
                
                for jogo, sugestoes in st.session_state.hedge_sugestoes.items():
                    st.markdown(f"#### 🎯 {jogo}")
                    
                    # Criar 3 colunas para as 3 sugestões
                    cols = st.columns(3)
                    
                    for idx, sugestao in enumerate(sugestoes[:3]):
                        with cols[idx]:
                            # Card da sugestão
                            tipo_emoji = {
                                'ESCANTEIO_CASA': '🏠',
                            ...
                            <div style="
                                ...
                                min-height: 200px;
                            ">
                                <h4>{tipo_emoji} Opção {idx+1}</h4>
                                <p style="font-size: 16px; font-weight: bold;">{sugestao['mercado']}</p>
                                <p>📊 Probabilidade: <strong>{sugestao['prob']:.1f}%</strong></p>
                                <p>{confianca_cor} Confiança: <strong>{sugestao['confiabilidade']}</strong></p>
                                <p style="font-size: 12px; margin-top: 10px;">📈 {sugestao['analise']}</p>
                            </div>
                            """, unsafe_allow_html=True)
                            
                            # Botão para adicionar ao hedge
                            if st.button(f"➕ Adicionar Hedge {idx+1}", key=f"add_hedge_{jogo}_{idx}"):
                                # Criar aposta de hedge
                                odd_estimada = 100 / sugestao['prob'] if sugestao['prob'] > 0 else 2.0
                                odd_estimada = min(max(odd_estimada, 1.5), 5.0)  # Limitar entre 1.5 e 5.0
                                ...
                                nova_aposta = {
                                    ...
                                    'prob': sugestao['prob'],
                                    'tipo': f'Hedge Inteligente'
                                }
                                
                                # Inicializar lista de hedges se não existir
                                if not hasattr(st.session_state, 'hedges_criados'):
                                    st.session_state.hedges_criados = []
                                
                                st.session_state.hedges_criados.append(nova_aposta)
                                st.success(f"✅ Hedge {idx+1} adicionado!")
                                time.sleep(0.3)
                                st.rerun()
            ...
            # ===== HEDGES CRIADOS =====
            if hasattr(st.session_state, 'hedges_criados') and st.session_state.hedges_criados:
                st.markdown("---")
                st.markdown("### 🛡️ Seus Bilhetes de Hedge")
                
                # Agrupar hedges em bilhetes (1 aposta por bilhete)
                for idx, hedge in enumerate(st.session_state.hedges_criados, 1):
                    with st.expander(f"🛡️ Hedge {idx}: {hedge['mercado']}", expanded=True):
                        col1, col2, col3, col4 = st.columns(4)
                        col1.markdown(f"**{hedge['jogo']}**")
                        col2.metric("Mercado", hedge['mercado'])
                        col3.metric("Odd", f"{hedge['odd']:.2f}")
                        col4.metric("Prob", f"{hedge['prob']:.0f}%")
                        
                        # Stake sugerido
                        banca = st.session_state.bankroll_history[-1]
                        stake_sugerido = banca * 0.02  # 2% da banca
                        
                        st.markdown(f"**💰 Stake Sugerido:** R$ {stake_sugerido:.2f} (2% da banca)")
                        
                        # Botão remover
                        if st.button(f"🗑️ Remover Hedge {idx}", key=f"remove_hedge_{idx}"):
                            st.session_state.hedges_criados.pop(idx-1)
                            st.rerun()
                
                st.markdown("---")
                
                # Análise geral
                st.markdown("### 📊 Análise Geral dos Hedges")
                
                total_hedges = len(st.session_state.hedges_criados)
                odd_media_hedges = np.mean([h['odd'] for h in st.session_state.hedges_criados])
                prob_media_hedges = np.mean([h['prob'] for h in st.session_state.hedges_criados])
                
                m1, m2, m3 = st.columns(3)
                m1.metric("🛡️ Total Hedges", total_hedges)
                m2.metric("📊 Odd Média", f"{odd_media_hedges:.2f}")
                m3.metric("🎯 Prob. Média", f"{prob_media_hedges:.1f}%")
                
                # Botão limpar todos
                if st.button("🗑️ Limpar Todos os Hedges", use_container_width=True):
                    st.session_state.hedges_criados = []
                    st.rerun()
            
            else:
                st.info("💡 Clique em 'Gerar Hedges Inteligentes' para começar!")
