...
def main():
    ...
    with main_menu[2]:
        ...
        if prompt := st.chat_input("Pergunte sobre um jogo (ex: Arsenal vs Chelsea)..."):
            ...
            # Processamento Inteligente V31 (Busca e Resposta)
            resp = "❌ Time não encontrado. Tente digitar o nome exato (ex: Arsenal)."
            
            # Normalização e busca de entidades (Times)
            clean_msg = prompt.replace("?", "").replace(".", "").title() # Capitaliza
            found = []
            ...
            if len(found) >= 1:
                t = found[0]; s = STATS_DB[t]
                resp = f"📊 **{t}**: {s['corners']:.1f} Cantos | {s['cards']:.1f} Cartões | {s['goals_f']:.1f} Gols."
