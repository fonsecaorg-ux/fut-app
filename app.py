...
║  Versão: V14.10                                                           ║
║  Data: Dezembro 2025                                                      ║
║  Novidade: Bloqueio de correlação (Ex: Home Corners vs Total Corners)     ║
╚═══════════════════════════════════════════════════════════════════════════╝
"""
...
def is_correlated(main_bet: Dict, candidate_bet: Dict) -> bool:
    ...
    if main_bet['market'] == candidate_bet['market']:
        ...
        # Se um for Home/Away e o outro for Total no mesmo mercado -> CORRELACIONADO (RUIM PARA HEDGE)
...
def generate_dual_hedges(main_slip: List[Dict], stats: Dict, refs_db: Dict):
    ...
    for gid, sels in games.items():
        ...
        # Fallback se não achou descorrelacionado
        if len(current_h1) < 2:
            leftover = [o for o in valid_opts if o['label'] not in main_labels][:2]
            for l in leftover: 
                if l not in current_h1: current_h1.append(l)
        ...
        if len(current_h2) < 2:
             ...
             for l in remaining:
                 if l not in current_h2: current_h2.append(l)
