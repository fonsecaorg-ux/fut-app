...
def parse_bilhete_texto(texto:str)->List[Dict]:
    """Parse texto colado do bilhete - ULTRA INTELIGENTE V3 - JUNTA LINHAS"""
    import re
    
    linhas_originais = [l.strip() for l in texto.split('\n') if l.strip()]
    
    # PRÉ-PROCESSAMENTO: Juntar linhas de mercado que foram quebradas
    linhas = []
    i = 0
    while i < len(linhas_originais):
        linha = linhas_originais[i]
        
        # Se linha atual é mercado SEM número E próxima tem número, juntar!
        if i + 1 < len(linhas_originais):
            proxima = linhas_originais[i + 1]
            
            # Linha atual tem mercado mas não tem número?
            tem_mercado_atual = any(x in linha.lower() for x in ['canto', 'escanteio', 'cartão', 'cartões', 'card', 'corner'])
            tem_numero_atual = bool(re.search(r'\d+\.5', linha))
            tem_numero_proximo = bool(re.search(r'\d+\.5', proxima))
            tem_mais_de_proximo = 'mais de' in proxima.lower() or 'over' in proxima.lower()
            
            if tem_mercado_atual and not tem_numero_atual and tem_numero_proximo and tem_mais_de_proximo:
                # Juntar as duas linhas!
                linha_juntada = linha + ' ' + proxima
                linhas.append(linha_juntada)
                i += 2  # Pular a próxima já que juntamos
                continue
    ...
    for linha in linhas:
        ...
        if any(x in linha.lower() for x in ['criar aposta', 'stake', 'retorno', 'odd total']):
            continue
        ...
        if ' vs ' in linha or ' x ' in linha.lower():
            ...
            if len(partes) == 2:
                jogo_atual = {
                    'home': partes[0].strip(),
                    'away': partes[1].strip(),
                    'mercados': mercados_pendentes.copy()
                }
        ...
        # Detectar mercado - COM ACENTOS CORRIGIDOS
        if any(x in linha.lower() for x in ['total de', 'total', 'mais de', 'over', 'under', 'menos de']) and \
           any(y in linha.lower() for y in ['canto', 'escanteio', 'cartão', 'cartões', 'cartoes', 'card', 'corner']):
            
            # Extrair tipo
            if any(x in linha.lower() for x in ['canto', 'escanteio', 'corner']):
                tipo = 'corners'
            elif any(x in linha.lower() for x in ['cartão', 'cartões', 'cartoes', 'card']):
                tipo = 'cards'
            else:
                tipo = 'cards'
            ...
            if nums:
                ...
                loc = 'total'
                if jogo_atual:
                    if jogo_atual['home'].lower() in linha.lower():
                        loc = 'home'
                ...
                # Adicionar ao jogo atual ou aos pendentes
                if jogo_atual:
                    jogo_atual['mercados'].append(mercado)
                else:
                    mercados_pendentes.append(mercado)
                continue
        
        # Formato sem "vs" - duas linhas consecutivas
        if not any(x in linha.lower() for x in ['total', 'mais de', 'menos de', 'over', 'under']) and len(linha) > 2:
...
def main():
    ...
    with tab8:
        ...
        with st.expander("📖 Como usar"):
            ...
            **DICA:** Cole exatamente como está na sua casa de apostas!
            """)
...
Formato 2 (sem vs - duas linhas):
Torino FC
Cagliari
Total de Cartões: Mais de 3.5
Total de Escanteios: Mais de 3.5

Formato 3 (misto):
Parma vs Fiorentina
Total de Cartões: Mais de 3.5 @1.33

Cole qualquer um desses formatos!""",
            key="bilhete_texto"
        )
